...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        nothing_path = resource_path(f'{self.exe_dir}/{self.pr_dir}/nothing.json')

        print(not os.path.exists(f"presets/"))
        if not os.path.exists(f"{self.pr_dir}/"):
            ...
            with open(resource_path(default_path), 'w') as json_file:
                json.dump({"sets": [], "sands": [], "goblet": [], "circlet": [], "sands_d": [], "goblet_d": [], "circlet_d": [], "substats": []}, json_file, ensure_ascii=False)
            # if not os.path.isfile(nothing_path):
            with open(resource_path(nothing_path), 'w') as json_file:
                json.dump({"sets": [], "sands": [], "goblet": [], "circlet": [], "sands_d": [], "goblet_d": [], "circlet_d": [], "substats": []}, json_file, ensure_ascii=False)
            print("Files 'config.json', 'default.json', 'nothing.json' created")
    ...
    def change_preset_event(self, preset: str):
        ...
        if s != "nothing":
            try:
                ...
                print(f'Файл {file_to_open} успешно загружен.')
            except FileNotFoundError:
                print(f'Файл {file_to_open} не найден.')
    ...
    def save_preset_event(self):
        ...
        if s == "nothing":
            ...
            if s in self.presets:
                ...
                # # Проверьте, существует ли директория, и создайте её, если она не существует
                # if not os.path.exists(output_dir):
                #     os.makedirs(output_dir)

                # Затем сохраняйте JSON-файлы в этой директории
                with open(resource_path(f'{self.exe_dir}/{self.pr_dir}/{s}.json'), 'w') as json_file:
                    json.dump(data, json_file, ensure_ascii=False)
            ...
            filep = os.path.abspath(f"{s}.json")
            print(f"saved: {s}   ({filep})")
            ...
            + goblet: {self.tab_2_selected}
            + circlet: {self.tab_3_selected}
            - sands: {self.tab_1_selected2}
            - goblet: {self.tab_2_selected2}
            - circlet: {self.tab_3_selected2}
            + substats: {self.substats_selected}
            ''')
    ...
    def delete_preset_event(self):
        ...
        if d == "nothing" or d == "default":
            ...
            file_to_delete = f"{d}.json"

            try:
                os.remove(resource_path(f'{self.exe_dir}/{self.pr_dir}/{file_to_delete}'))
    ...
    def start_button_event(self):
        ...
        print(f"| Соотношение сторон экрана: {screen_ratio}")
